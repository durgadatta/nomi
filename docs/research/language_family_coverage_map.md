...
| Array languages | APL, J, K, Q, BQN, Uiua, Julia broadcasting | Whole-data thinking, rank/shape, dense transformation | future rank/shape functions, readable whole-array operations | Glyph density and tacit code are not the default Nomi style. |
| Numerical/scientific languages | Julia, R, MATLAB, Mathematica | Notebooks, broadcasting, multiple dispatch, symbolic/math workflows | data notebooks, dispatch research, explicit symbolic boundaries | Domain power may conflict with general-purpose readability. |
