...
| Core IR becomes decorative | High → Medium-Low | The project can point at Core IR while real semantics still flow through Python AST. Mitigated by `NOMI_VERIFY_CORE=1`, explicit `core-runtime` and `js-core-runtime` eval backends, serialized Core IR JSON, and the shared backend fixture ladder under `prototype/tests/backend_fixtures/`. | Future backends and diagnostics inherit a misleading artifact boundary if Surface -> Core remains a Python AST back-projection too long. | `NOMI-ARCH-019`: make Surface -> Core lowering authoritative for more constructs and keep expanding cross-backend fixtures. |
...
| Cache identity is under-specified | High | Parser caches are typed now, but feature-profile and source-version values are placeholders until target parsing arrives. | Cache invalidation bugs lead to false performance wins or stale execution. | `NOMI-SUBSTRATE-029`: feed real profile/source-version inputs into typed keys. |
