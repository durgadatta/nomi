...
| Core IR becomes decorative | High → Medium | The project can point at Core IR while real semantics still flow through Python AST. Partially mitigated: `NOMI_VERIFY_CORE=1` and `NOMI_USE_CORE_IR=1` env vars now gate a concrete Core IR execution path, and `prototype/runtime/backends/core_direct.py` proves direct CoreNode dispatch works. | Future backends and diagnostics inherit a misleading artifact boundary. | `NOMI-ARCH-019`: make Surface -> Core lowering authoritative for more constructs (BinOp, etc. still produce Diagnostic nodes). |
