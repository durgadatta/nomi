...
| NOMI-SUBSTRATE-002 | Feature-set parser API | 🟡 partial | `extra_layers` is useful but stringly and grammar-only; parser technology was implicit. | `get_parser(features=[...])` should select feature manifests and parser frontend profiles, not just raw layer files. | `ParserFrontendSpec` and `LarkParserFrontend` now wrap the current path; next add feature/profile selection above this boundary. |
...
| NOMI-SUBSTRATE-029 | Parser/cache identity | 🟡 partial | ~~Parser and raw-tree caches are not keyed by feature profile, grammar version, source path, or source version.~~ | Cache keys represent parser frontend, profile, feature set, grammar assembly, source identity, and position mode. | `ParserCacheKey` and `RawTreeCacheKey` now cover frontend, grammar layers, grammar version, feature profile placeholder, source identity, and position mode. Remaining: real feature-profile/source-version inputs for docs-only and target-tour parsing. |
...
| NOMI-SUBSTRATE-002 | `prototype/parser/nomi/frontend.py`, `prototype/parser/nomi/usage.py` | Parser technology is now wrapped by `ParserFrontendSpec`; parser API still needs real feature/profile selection above the Lark-compatible facade. |
...
| NOMI-SUBSTRATE-029 | `prototype/parser/nomi/frontend.py` | Parser/raw-tree caches include the frontend name; they still need full profile/source-version identity before docs-only or target-tour parsing. |
