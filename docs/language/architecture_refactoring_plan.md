...
| NOMI-ARCH-004 | Structured results | `ExecutionResult` carries bindings, value, stdout/stderr, passive diagnostics/events, timings, and optional exception. | Stage artifacts and detailed timings flow through the same result shape. | Add per-stage artifact/timing fields after frontend adapters settle. |
...
| NOMI-ARCH-013 | Semantic event collection | Passive `Diagnostic`, `RuntimeEvent`, and `RuntimeEventCollector` records exist, but no runtime stage produces events yet. | A passive event sink collects semantic events without changing presentation or control flow. | Route parser/lowering/runtime through the collector one feature at a time. |
...
| NOMI-ARCH-015 | Runtime cache identity | `RuntimeSession` caches ASTs with `RuntimeCacheKey`: source text, source identity, mode, profile, parser, lowering, grammar version, and span mode. | Cache invalidation follows the same typed key as parser/profile support grows. | Add feature-profile and parser-version inputs when those become selectable. |
...
| NOMI-ARCH-019 | Core IR and verifier | Passive Core IR nodes and inspection exist, but current lowering projects backward from Python AST. | A Nomi Core IR with verifier defines reference semantics before backend lowering. | Add an authoritative tiny Surface -> Core lowering path before diagnostics or backend work rely on Core IR. |
...
| NOMI-ARCH-023 | CLI runtime facade migration | CLI uses `prototype.runtime.execute()` and prints captured stdout/stderr plus bindings. | CLI displays structured diagnostics/errors once those records exist. | Add CLI contract tests for structured failure output after diagnostic records are produced. |
| NOMI-ARCH-024 | Frontend result contract | Web consumes captured `RuntimeSession` output; notebook still streams through kernel redirects. | Web, notebook, CLI, and future REPL adapt one `ExecutionResult` diagnostics/events/output contract. | Migrate notebook after result-backed stdout/stderr display and Jupyter streaming expectations are reconciled. |
