...
| NOMI-ARCH-023 | CLI runtime facade migration | CLI still calls compatibility `run_eval_loop` modules directly. | CLI uses `prototype.runtime.execute()` and displays structured output/errors. | Move after `ExecutionResult` owns stdout/stderr, diagnostics, structured errors, and exit behavior. |
| NOMI-ARCH-024 | Frontend result contract | Web and notebook adapt runtime output/errors through private shapes. | Web, notebook, CLI, and future REPL adapt one `ExecutionResult` diagnostics/events/output contract. | Add passive fields first, then migrate one adapter with contract tests. |
