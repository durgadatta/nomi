...
def test_where_later_uses_earlier_binding(interpreter_mode):
    run = get_run_eval_loop(interpreter_mode)
    bindings = run(code="result = z where:\n    x = 10\n    y = x * 2\n    z = y + 5\n")
    assert bindings["result"] == 25


def test_where_mutual_recursion(interpreter_mode):
    run = get_run_eval_loop(interpreter_mode)
    bindings = run(code="\n".join([
...
def test_where_mutual_recursion_odd(interpreter_mode):
    run = get_run_eval_loop(interpreter_mode)
    bindings = run(code="\n".join([
